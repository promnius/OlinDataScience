"""
Make SQL database useful for Python
"""
...
class Events(Table):
    ...
    def ReadRecords(self, password = None, username = None):
        self.ReadDatabase(self.GetFields(), Event, "INNER JOIN current_system_infos ON event_infos.sn=current_system_infos.sn WHERE product_number IS NOT NULL ORDER BY event_infos.timestamp", username = username, password = password)
...
class Errors(Table):
    ...
    def ReadRecords(self, password = None, username = None):
        self.ReadDatabase(self.GetFields(), Event, "INNER JOIN current_system_infos ON event_infos.sn=current_system_infos.sn INNER JOIN facilities ON facilities.id = current_system_infos.facility_id", username = username, password = password)

    def GetFields(self):
        return [
            "id", "sn", "event", "event_id", "description", "timestamp", "event_type", "created_at", "received_at", "facility_id", "name", "product_number"
            ]
